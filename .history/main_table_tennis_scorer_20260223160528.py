...
class TableTennisScorer:
    ...
    def process_score(self, source, target_side=None):
        ...
        if self.score_a >= 10 and self.score_b >= 10 and score_diff >= 2:
            ...
            print(log_msg)
            if self.tts_available:
                final_text = f"{winner}方获得本局胜利，总比分A{self.total_games_a}比B{self.total_games_b}"
                if match_winner:
                    final_text = f"{match_winner}方赢得整场比赛，最终比分A{self.total_games_a}比B{self.total_games_b}"
                self._speak_async(final_text)
            if LOG_TO_FILE:
                logging.info(log_msg)
            return True
        # ======== 普通胜负判定（10分前） ========
        elif (self.score_a >= WINNING_SCORE or self.score_b >= WINNING_SCORE) and score_diff >= MINIMUM_WINNING_DIFFERENCE and max_score < 10:
            winner = 'A' if self.score_a > self.score_b else 'B'
            if winner == 'A':
                self.total_games_a += 1
            else:
