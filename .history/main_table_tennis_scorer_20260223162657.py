    ...
    def draw_ui(self, frame):
        ...
        if self.voice_recognition_available:
            ...
            if self.voice_state == 'listening':
                # 录音进度条：线性增长
                progress = min(elapsed / VOICE_PHRASE_TIME_LIMIT, 1.0)
                fill_w = int(voice_bar_w * progress)
                bar_color = (0, 220, 100)
                cv2.rectangle(frame, (voice_bar_x, voice_bar_y),
                ...
                cv2.circle(frame, (mic_cx, mic_cy), 8, (0, 220, 100), -1)
                cv2.circle(frame, (mic_cx, mic_cy), 8, (0, 255, 0), 2)
                self._put_text_centered(frame, '录音中...', mid_x, voice_label_y, 0.7, (0, 220, 100), 2)

            elif self.voice_state == 'processing':
                # 识别中：橙色跑马灯
                cycle = (elapsed * 1.8) % 1.0
                seg_w = voice_bar_w // 3
                ...
                self._put_text_centered(frame, '识别中...', mid_x, voice_label_y, 0.7, (0, 200, 255), 2)
