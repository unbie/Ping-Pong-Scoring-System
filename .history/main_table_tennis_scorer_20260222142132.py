...
try:
    ...
    print("Warning: speech_recognition or pyaudio module not found, voice recognition feature unavailable")
    SPEECH_RECOGNITION_AVAILABLE = False
except AttributeError as e:
    if "Could not find PyAudio" in str(e):
        print("Warning: PyAudio not found, voice recognition feature unavailable")
        SPEECH_RECOGNITION_AVAILABLE = False
    else:
        raise e
...
class TableTennisScorer:
    def __init__(self):
        ...
        if self.voice_recognition_available:
            try:
                ...
                print("Voice recognition feature enabled")
            except Exception as e:
                print(f"Voice recognition initialization failed: {str(e)}")
                self.voice_recognition_available = False
        else:
            self.voice_recognition_available = False
            print("Voice recognition feature unavailable")
        ...
        while True:
            try:
                with self.microphone as source:
                    # 每次识别前调整环境噪音
                    self.recognizer.adjust_for_ambient_noise(source)
                    audio = self.recognizer.listen(source, timeout=1, phrase_time_limit=VOICE_PHRASE_TIME_LIMIT)
                try:
                    # 使用Google语音识别
                    ...
                    # Check if it contains keywords for adding scores to specific players (convert to lowercase for comparison)
                    text_lower = text.lower()
                    
                    # Check if specify adding score to player A or player B
                    if any(keyword in text_lower for keyword in ['a score', 'a point', 'left score', 'player a']):
                        self.process_score('voice', 'A')
                    elif any(keyword in text_lower for keyword in ['b score', 'b point', 'right score', 'player b']):
                        self.process_score('voice', 'B')
                    else:
                        # If no specific instruction matched, continue listening
                        continue
                except sr.UnknownValueError:
                    # Could not recognize the voice, continue listening
                    continue
    ...
    def process_score(self, source, target_side=None):
        ...
        if current_time - self.last_score_time < self.cooldown_period:
            if DEBUG_MODE:
                print("Still in cooldown period, ignoring this score")
            return False
        ...
        print(log_msg)  # Removed Chinese characters to prevent encoding issues
        ...
        if winning_score_reached and winning_diff_reached:
            ...
            log_msg = f"Game Over! Player {winner} wins! Final Score A:{self.score_a} - B:{self.score_b}"
            print(log_msg)  # Removed Chinese characters to prevent encoding issues
    ...
    def draw_ui(self, frame):
        ...
        # 显示语音识别状态
        voice_status = "Voice: Enabled" if self.voice_recognition_available else "Voice: Unavailable"
        ...
        # 显示操作说明
        cv2.putText(frame, "Keys: 'q'-Quit | 'r'-Reset | 's'-Switch Gesture", (20, h - 20), 
                   cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE * 0.6, (200, 200, 200), 1)
        
        # 显示系统说明
        gesture_desc = "Gesture: Scissors or Touch Action"
        voice_desc = "Voice: Say 'A Score/B Score'" if self.voice_recognition_available else "Voice: Unavailable"
    ...
    def run(self):
        ...
        if self.voice_recognition_available:
            print(f"Use 'A Score/B Score' or gestures to score")
        else:
            print("Use gestures to score")
