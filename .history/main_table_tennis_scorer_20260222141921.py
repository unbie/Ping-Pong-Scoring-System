...
class TableTennisScorer:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    # 检查是否包含给特定选手加分的关键词（转换为小写进行比较）
                    text_lower = text.lower()
                    
                    # 检查是否指定给A方或B方加分
                    if any(keyword in text_lower for keyword in ['a队得分', 'a方得分', '左边得分', '左方得分', 'a score', 'a point']):
                        self.process_score('voice', 'A')
                    elif any(keyword in text_lower for keyword in ['b队得分', 'b方得分', '右边得分', '右方得分', 'b score', 'b point']):
                        self.process_score('voice', 'B')
                    else:
                        # 如果没有匹配到特定指令，则不执行任何操作
                        continue
    ...
    def process_score(self, source, target_side=None):
        ...
        log_msg = f"得分！选手{scoring_player}得分，来源：{source}，当前比分 A:{self.score_a} - B:{self.score_b}"
    ...
    def draw_ui(self, frame):
        ...
        voice_desc = "语音: 说 'A队得分/B队得分'" if self.voice_recognition_available else "语音: 不可用"
    ...
    def run(self):
        ...
        if self.voice_recognition_available:
            print(f"请使用'A队得分/B队得分'等关键词或做出得分手势来得分".encode('utf-8').decode('utf-8'))
        else:
